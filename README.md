...
| **검증** | pytest 228 케이스 통과(LLM · 외부 API 모킹, GitHub Actions CI), 배포 · 운영 이슈 15건을 로그로 원인 추적해 기록([7절](#7-기술적-도전과-해결)), 로컬 실제 실행으로 게이트 동작 확인([실제 실행 기록](#실제-실행-기록-2026-09-19)) |
| **측정** | 리포트 20건 반복 생성 → 게이트 정상 통과 **0/20**. 원인(LLM 편집장이 오늘 날짜를 몰라 51번 중 0번 통과)을 찾아 프롬프트에 날짜 · 기준 시각 · 알려진 한계를 넣고 재측정 → **5/20**, 편집장 통과 5/47, 건당 평균 $0.012 · 78초([측정 기록](#반복-측정-20건-2026-09-19)) |
| **한계** | 리포트 정상 통과 5/20(남은 원인: 상대 시각 표현 · 삼성전자 형식 게이트), 폴백 저장은 편집장 미경유, 결제는 mock ([13절](#13-한계와-다음-단계)) |
...
> 로컬에서 실행한 화면입니다(2026-09-19, 실시간 시세는 9개 핵심 티커, 나머지는 데모 데이터). 리포트의 `기준 시각 … 04:19:(수치 미확인)`은 아래 실행 기록에서 찾은 결함이 드러난 부분입니다(수정 전 실행 결과, 지금은 수정됨).
...
| 거부(미저장) | 15 / 20 | 10 / 20 |
| LLM 편집장 PASS / 호출 | 0 / 51 | 5 / 47 |
| 건당 평균 비용 · 시간 · LLM 호출 | $0.0127 · 83초 · 15.4회 | $0.0120 · 78초 · 14.4회 |

- **원인**: 편집장 프롬프트가 "올해 최신성"만 요구하고 오늘 날짜 · 데이터 기준 시각을 주지 않아, 모델이 2026년 데이터를 "현재와 차이가 있다"(한 번은 "현재 시점이 2023년")며 거부했습니다. 저장된 5건은 모두 편집장을 거치지 않는 숫자 정제 폴백이었습니다.
- **수정**: 게이트를 끄거나 느슨하게 하지 않고, 편집장에게 오늘 날짜 · 기준 시각 · 알려진 데이터 한계를 주고 "한계가 명시되어 있으면 그 자체로는 FAIL이 아니다"를 명시했습니다([변경 기록](docs/harness/records/ai-report/report-evaluator-date-context-2026-09-19.md)).
- **남은 문제**: 편집장 거부 사유가 날짜 오판에서 "'6시간 전' 같은 상대 시각 표현이 모호하다"는 내용 지적으로 바뀌었습니다. 삼성전자(005930.KS)는 0/4로 형식 게이트에서 계속 실패합니다. 20건 표본이라 실행마다 편차가 있고, 운영 서버 수치가 아닙니다([처음 측정 기록](docs/harness/records/ai-report/report-generation-measurement-2026-09-19.md)).
...
- **CI**: GitHub Actions(`backend-tests`)가 push · PR마다 `.env` 없이 더미 설정으로 pytest를 실행합니다. CI를 붙이며 로컬 `.env`에 API 키가 있어야만 통과하던 테스트 1건을 찾아 고쳤습니다([기록](docs/harness/records/project/ci-backend-tests-2026-09-19.md)).
...
- **테스트·환경 관리**
  ...
  - `requirements.txt`는 2026-09-19에 테스트 · 실제 실행 버전으로 고정했습니다. 잠금 파일(uv) 전환은 계획 단계입니다([계획](docs/harness/records/deployment/uv-migration-plan-2026-06-03.md)).
- **리포트 품질**: 반복 측정 정상 통과 5/20(편집장 수정 전 0/20). 다음 과제는 writer가 상대 시각("6시간 전") 대신 절대 시각을 쓰게 하는 것, 삼성전자 형식 게이트 실패 원인 분석, 폴백 저장본도 편집장 판정을 기록하는 것입니다([변경 기록](docs/harness/records/ai-report/report-evaluator-date-context-2026-09-19.md)).
