...
| **검증** | pytest 227 케이스 통과(LLM · 외부 API 모킹, GitHub Actions CI), 배포 · 운영 이슈 15건을 로그로 원인 추적해 기록([7절](#7-기술적-도전과-해결)), 로컬 실제 실행으로 게이트 동작 확인([실제 실행 기록](#실제-실행-기록-2026-09-19)) |
| **측정** | 리포트 20건 반복 생성: 건당 평균 **$0.013 · 83초 · LLM 15회**. 게이트 정상 통과 **0/20**, 폴백 저장 5 · 거부 15 — LLM 편집장이 51번 호출돼 한 번도 통과시키지 않은 것이 주원인([측정 기록](#반복-측정-20건-2026-09-19)) |
| **한계** | 편집장 판정 기준 개선 전(위 측정), 결제는 mock ([13절](#13-한계와-다음-단계)) |
...
실행 한두 건으로는 통과율과 비용을 알 수 없어서, 스케줄러 대상 5개 자산을 4번씩 같은 조건(로컬 · 임시 DB · 실제 `gpt-4o-mini` · 시세 API)으로 생성했습니다.

| 지표 | 값 |
| --- | --- |
| 게이트 정상 통과 | **0 / 20** |
| 숫자 정제 폴백으로 저장 | 5 / 20 |
| 거부(미저장) | 15 / 20 — 마지막 사유: LLM 편집장 11 · 형식 2 · 정성 1 · 숫자 1 |
| 건당 비용 · 시간 · LLM 호출 | 평균 $0.0127 · 83초 · 15.4회 (20건 합계 $0.255) |

- **LLM 편집장이 51번 호출되어 한 번도 통과시키지 않았습니다.** 프롬프트가 "올해 최신성"만 요구하고 오늘 날짜 · 데이터 기준 시각을 주지 않아, 모델이 2026년 데이터를 "현재와 차이가 있다"(한 번은 "현재 시점이 2023년")며 거부했습니다.
- 저장된 5건은 모두 숫자 정제 폴백 경로로, 이 경로는 편집장을 다시 부르지 않습니다. 즉 지금 설정에서는 **편집장을 통과한 리포트가 하나도 없습니다.**
- 개선(편집장에 날짜 · 기준 시각 제공, 데이터 한계 표기 기준 명시)은 아직 적용하지 않았습니다. 상세: [측정 기록](docs/harness/records/ai-report/report-generation-measurement-2026-09-19.md)
...
- **백엔드 테스트**: pytest + pytest-asyncio로 작성했습니다. **24개 파일 · 227 케이스 전부 통과**합니다.
...
| 테스트 | pytest, pytest-asyncio (24개 파일 · 227 케이스) |
...
- **리포트 품질**: 반복 측정에서 정상 통과 0/20. LLM 편집장 프롬프트에 오늘 날짜 · 데이터 기준 시각이 없어 "최신성"을 이유로 계속 거부하고, 폴백 저장본은 편집장을 거치지 않습니다. 개선 후보는 [측정 기록](docs/harness/records/ai-report/report-generation-measurement-2026-09-19.md)에 있습니다.
