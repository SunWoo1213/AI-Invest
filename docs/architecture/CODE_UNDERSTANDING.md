...
> 이 문서는 요약 지도이며, 세부 규칙과 최신 변경 이력은 [AGENTS.md](../../AGENTS.md), [docs/harness/feature-index.md](../harness/feature-index.md), 각 `docs/harness/features/*.md`가 진실의 소스이다. 문서와 코드가 충돌하면 **현재 코드가 우선**한다.
...
| 배포 | Frontend → Vercel, Backend → Render/Supabase(시험 중) | [VERCEL_SUPABASE_INTEGRATION_GUIDE.md](../guides/VERCEL_SUPABASE_INTEGRATION_GUIDE.md) |
...
│  └─ package.json
├─ docs/
│  ├─ architecture/               # 이 문서, ARCHITECTURE(일부 구식), 구조 분석, 기능 상세 명세
│  ├─ guides/                     # 환경변수·Gmail·Telegram·Stooq·Vercel/Supabase 설정 가이드
│  ├─ deliverables/               # 캡스톤 최종 산출물 7종
│  └─ harness/                    # feature-index, features/, records/<영역>/ 변경 기록
├─ scripts/                       # 환경변수 문서 동기화 검사기, test_api.py·test_db.py 수동 점검 헬퍼
...
수동 점검 헬퍼(`scripts/`): [test_api.py](../../scripts/test_api.py)(OpenAI 키/LangChain 연결), [test_db.py](../../scripts/test_db.py)(DB 조회·ORM 동작).
...
**이 문서(`docs/architecture/CODE_UNDERSTANDING.md`)는 저장소의 구조·데이터 흐름을 설명하는 상위 지도이므로, 코드 구조가 바뀌면 반드시 함께 최신화한다.** 하네스 워크플로우(plan → implement → verify → 문서화)의 문서화 단계에서 다음을 점검한다.
...
2. 같은 변경에 대해 [docs/harness/](../harness/)에 한국어 변경 기록을 남기고, 관련 `docs/harness/features/*.md`와 [docs/harness/feature-index.md](../harness/feature-index.md)를 갱신한다(상세: [docs/harness/feature-documentation-guide.md](../harness/feature-documentation-guide.md)).
...
- 2026-06-10: 최초 작성. 전체 구조·데이터 흐름 정리. (변경 기록: [docs/harness/records/project/code-understanding-doc-2026-06-10.md](../harness/records/project/code-understanding-doc-2026-06-10.md))
- 2026-06-10: 챗봇(§3.8)·알림 Gmail/Telegram(§3.9) 상세 보강, Toss 결제 미구현 상태 명시(§1, §3.2, §3.3, §5.4). (변경 기록: [docs/harness/records/project/code-understanding-doc-chatbot-notification-toss-2026-06-10.md](../harness/records/project/code-understanding-doc-chatbot-notification-toss-2026-06-10.md))
- 2026-09-17: 문서 재배치(`docs/architecture/`로 이동) 및 §2 구조 트리 갱신. (변경 기록: [repository-docs-reorganization-2026-09-17.md](../harness/records/project/repository-docs-reorganization-2026-09-17.md))
