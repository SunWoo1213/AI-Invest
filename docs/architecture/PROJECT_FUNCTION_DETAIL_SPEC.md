...
> Harness note (2026-05-30): 이 문서는 제품 기능을 넓게 설명하는 기존 명세입니다. 현재 코드 기준의 하네스 작업 문서는 `docs/harness/feature-index.md`와 `docs/harness/features/`를 우선합니다. 특히 인증은 현재 Google-only 흐름이므로 `docs/harness/features/authentication.md`와 `docs/harness/records/auth/google-login-only.md`를 기준으로 판단합니다.
...
주의 사항 : 기존 `docs/architecture/ARCHITECTURE.md`, `docs/architecture/PROJECT_STRUCTURE_ANALYSIS.md` 파일 일부는 인코딩이 깨져 있어, 본 명세서는 실제 코드 동작 구조를 우선 기준으로 작성하였다.
...
`docs/architecture/ARCHITECTURE.md` : 프로젝트 아키텍처 설명 문서이나, 현재 일부 텍스트 인코딩이 깨져 있다.

`docs/architecture/PROJECT_STRUCTURE_ANALYSIS.md` : 기존 프로젝트 구조 분석 문서이나, 현재 일부 텍스트 인코딩이 깨져 있다.

`scripts/test_api.py` : OpenAI API 연결 테스트 용도의 루트 테스트 스크립트이다.

`scripts/test_db.py` : 특정 티커의 AI 리포트를 DB에서 조회하는 테스트 스크립트이다.
...
OpenAI 연결 테스트 : `scripts/test_api.py`는 `.env`의 OpenAI API Key를 로드하여 LangChain ChatOpenAI 호출이 가능한지 확인한다.

DB 조회 테스트 : `scripts/test_db.py`는 BTC-USD의 최신 AI 리포트가 DB에 저장되어 있는지 조회하는 스크립트이다.
