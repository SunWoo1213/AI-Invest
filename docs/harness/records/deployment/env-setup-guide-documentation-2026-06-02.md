...
- `docs/guides/ENVIRONMENT_VARIABLE_SETUP.md`에 `.env_example` 변수값을 얻기 위해 실제로 진행해야 하는 과정을 단계형으로 추가했다. 로컬 기본값, frontend/backend URL, Docker DB, hosted DB, JWT secret, Google OAuth, OpenAI, 시장 데이터 provider, 리포트 scheduler, 결제 provider, 알림 provider, 최종 검증 순서로 나누어 따라 할 수 있게 구성했다.
...
- `.env_example`, `docs/architecture/ARCHITECTURE.md`, `docs/architecture/PROJECT_STRUCTURE_ANALYSIS.md`, root `DEVELOPMENT_DIRECTION.md`, `docs/harness/feature-index.md`, `docs/harness/features/deployment-runtime.md`, `backend/app/core/config.py`, `frontend/src/utils/apiClient.js`를 참고했다.
