...
- 거부 목록에 예전 값 원문을 두면 현재 코드에 시크릿이 다시 공개되므로, `LEAKED_SECRET_KEY_SHA256`(SHA-256 해시)으로 바꿔 비교한다.
- `backend/tests/test_log_sanitizer.py`에 실제 공공데이터포털 `serviceKey`로 보이는 값이 있어 가짜 값으로 교체했다. 마스킹 동작 검증은 그대로다.
- 검증: `cd backend; pytest` → 227 passed, 예전 값 설정 시 `ValueError` 확인
- 두 값 모두 git 기록에는 남아 있다. 공공데이터포털 인증키는 재발급을 권장한다.
