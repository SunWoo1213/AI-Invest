...
- `frontend/.env` 파일 부재
  - 현재 저장소에는 루트 `.env`만 있고 `frontend/.env`는 없다. `frontend/`에서 직접 Vite를 실행하면 `VITE_GOOGLE_CLIENT_ID`가 주입되지 않으므로, 필요한 경우 `frontend/.env`에 `VITE_API_BASE_URL`, `VITE_GOOGLE_CLIENT_ID` 같은 공개값을 둔다(`docs/guides/ENVIRONMENT_VARIABLE_SETUP.md` 참고). 실제 값과 시크릿은 문서/커밋에 넣지 않는다.
