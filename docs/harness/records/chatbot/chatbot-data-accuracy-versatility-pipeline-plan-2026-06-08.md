...
- `backend/app/core/config.py`에 위 토글 추가. 신규 변수는 `docs/guides/ENVIRONMENT_VARIABLE_RECOMMENDATIONS.md`(이미 미추적 파일로 존재)와 배포 런타임 feature 문서에 기록.
...
- `docs/harness/features/deployment-runtime.md` 및 `docs/guides/ENVIRONMENT_VARIABLE_RECOMMENDATIONS.md` — 신규 환경변수(`CHATBOT_USE_TOOLS`, `CHATBOT_MAX_TOOL_ROUNDS`, `CHATBOT_RATE_LIMIT_PER_MIN`, `CHATBOT_GROUNDING_GUARD`) 기록.
- 구현 단계에서 `docs/harness/records/chatbot/chatbot-data-accuracy-versatility-pipeline-implementation-2026-06-08.md`(또는 Phase별) 변경 기록 작성.
