...
- `POST /api/ai/generate/{ticker}` was unauthenticated when this change record was first written; later auth work protects it with `get_current_user`. See `docs/harness/records/ai-report/report-quality-phase-1.md` and `docs/harness/records/auth/google-login-only.md`.
