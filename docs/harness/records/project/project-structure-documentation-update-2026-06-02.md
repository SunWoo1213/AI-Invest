...
- `docs/harness/records/project/project-structure-documentation-update-2026-06-02.md`

## Behavior Changes

- 코드 동작 변경 없음.
- `docs/architecture/ARCHITECTURE.md`가 현재 런타임, 프론트엔드 라우트, 백엔드 라우터/API, DB 모델, 스케줄러/AI 리포트 정책, 설정 경계를 설명하도록 갱신됨.
- `docs/architecture/ARCHITECTURE.md`를 한국어 중심 문서로 번역하고, 파일 경로/API 경로/환경변수명은 원문 식별자를 유지함.
- `docs/architecture/PROJECT_STRUCTURE_ANALYSIS.md`가 새로 추가되어 저장소 폴더별 책임과 기능 문서 탐색 경로를 빠르게 확인할 수 있음.
...
- `docs/architecture/ARCHITECTURE.md` 번역 후 문서 본문을 다시 읽어 주요 코드 식별자가 유지되는지 확인함.
