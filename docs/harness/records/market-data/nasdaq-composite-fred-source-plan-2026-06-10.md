...
| 문서(가이드) | `docs/guides/STOOQ_APIKEY_GUIDE.md` | "나스닥은 FRED로 이전. USD/KRW 등락은 삭제됨. **Stooq는 라이브 기본 경로에서 더 이상 사용하지 않음**(휴면 fallback만 잔존). `STOOQ_API_KEY` 수동 교체 불필요" 보강. |
...
- `docs/guides/STOOQ_APIKEY_GUIDE.md` — 나스닥은 FRED로 이전, USD/KRW 등락 삭제, Stooq는 라이브 기본 경로 미사용(휴면) 명시.
