...
### 설정/문서 (시크릿 값 미포함)
- [ENVIRONMENT_VARIABLE_RECOMMENDATIONS.md](docs/guides/ENVIRONMENT_VARIABLE_RECOMMENDATIONS.md) (이미 미추적 상태로 존재) 또는 env 가이드 — `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET` 항목과 발급 방법(네이버 개발자센터 애플리케이션 등록, 검색 API 사용 설정) 안내 추가. **실제 키 값은 기록하지 않음.**
