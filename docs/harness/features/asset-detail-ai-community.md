...
19. The graph evaluates format-, numeric-, and qualitative-check-passing reports. The LLM evaluator prompt receives today's UTC date, the report `data_as_of`, and the known data limitations (`data_limitations` + `missing_required_facts`); freshness is judged against those values rather than the model's training date, and a disclosed data limitation alone is not a FAIL reason (2026-09-19). Passing reports are saved with bull and bear summaries derived from role outputs when available.
...
- `docs/harness/records/ai-report/report-generation-measurement-2026-09-19.md`
- `docs/harness/records/ai-report/report-evaluator-date-context-2026-09-19.md`
